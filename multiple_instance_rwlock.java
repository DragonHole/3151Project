...
public class multiple_instance_rwlock {
    // global variables 
    ...
    public static class Reader extends Thread {
        ...
        public void run() {
            int r = 0;
            while(r < this.rounds){ 
                while(note[this.id] == true) { // repeat until a complete value of counter is obtained
                    //如果writer一口气过完一次loop，value++，但还是ok的？ ok
                    // 但是如果modify左，没set bit，reader还是会以为no writing occurred. 
                    note[this.id] = false; // writer interleave possible between this line and 1 above? 
                    for (int i = 0; i < NUM_BYTES; i++){ 
                        this.local_c_decoy[i] = c[i];
                    }
                    if(note[this.id] == false){   // no writing occured during the above 3 lines
                        for (int i = 0; i < NUM_BYTES; i++){
